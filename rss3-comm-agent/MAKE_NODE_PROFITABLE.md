...
### Postgres Configuration Optimizations (Applied)

The database configuration has been optimized to reduce space usage and improve performance:

**In `/home/babajohn33/rss3-node/config/config.yaml`:**

```yaml
database:
  driver: postgres          # Explicitly set database driver
  partition: true           # Enable table partitioning for better cleanup
  coverage_period: 3        # Minimum retention period (3 months)
  uri: postgresql://...     # Connection string (no invalid pool params)
```

**Ethereum Worker Optimizations:**
```yaml
component:
  decentralized:
  - id: ethereum-core
    parameters:
      concurrent_block_requests: 2    # Reduced from default
      block_batch_size: 10           # Smaller batches
      receipts_batch_size: 20         # Smaller batches
```

**Benefits:**
- **`partition: true`**: Enables automatic table partitioning, making cleanup more efficient
- **`coverage_period: 3`**: Sets minimum data retention (3 months), allowing automatic trimming of older data
- **Reduced batch sizes**: Prevents overwhelming the database during catch-up indexing
- **Smaller concurrent requests**: Reduces database load and connection pressure

**Note**: Connection pool parameters (`pool_max_conns`, `pool_min_conns`) cannot be set in PostgreSQL connection URIs - these are handled by connection poolers like pgBouncer or application-level pools.
